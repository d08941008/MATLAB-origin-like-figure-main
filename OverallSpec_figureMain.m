...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all; close all;
data = loadData('OverallSpec.txt', 1, ':'); data = [data, -25.5*ones(401, 1)];
data = copy_xvec(data); figname = 'test';

xlabel = 'Wavelength (μm)';
ylabel = 'Transmission (dB)';
mode = 2;       % 1 for taller figure (v-b curve), 2 for shorter figure (transmission)
legend = {};

% xticksCell = {1.25:0.02:1.35, 'auto'};
xticksCell = get_ticksCell(1.25:0.02:1.35, 1);       % 1st field: xticks(0.25:0.05:1.5); 2nd field: xticklabels('0.25', '', '', '', '', '0.5', ...), only ticks at index of 1, 6, 11, 16, ... are shown with label
% yticksCell = {1.46:0.04:1.82, 'auto'};
yticksCell = get_ticksCell(-40:10:0, 1);      % similar way at line 35; every 2 tick steps of ticks are shown with label
% yticksCell1 = get_ticksCell(1.46:0.04:1.82, 2);      
% yticksCell2 = get_ticksCell(3.16:0.04:3.5, 2);     
